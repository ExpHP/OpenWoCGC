...

{
    ...
    if (0 < count) {
        do {
            ...
            tex = NuTexCreate(&nuTex);
            numtids = scene->numtids;
            scene->tids[numtids] = (short)tex;
...

{
    ...
    if (0 < nus->nummtls) {
        iVar3 = 0;
        do {
            iVar1 = *(int*)((int)&nus->mtls->next + iVar3);
            if (*(int*)(iVar1 + 0x38) == -1) {
                *(undefined4*)(iVar1 + 0x38) = 0;
                *(undefined*)(*(int*)((int)&nus->mtls->next + iVar3) + 0x42) = 0;
            }
            else {
                *(int*)(iVar1 + 0x38) = (int)nus->tids[*(int*)(iVar1 + 0x38)];
                iVar1 = NuTexGetDecalInfo(*(int*)(*(int*)((int)&nus->mtls->next + iVar3) + 0x38));
                *(char*)(*(int*)((int)&nus->mtls->next + iVar3) + 0x42) = (char)iVar1;
            }
            iVar2 = iVar2 + 1;
            iVar3 = iVar3 + 4;
            NuMtlUpdate();
        } while (iVar2 < nus->nummtls);
    }
    return;
}
...
void ReadNuIFFGSplineSet(fileHandle handle, struct nugscn_s* scene)
...

{
    ...
    if (0 < ngobjs) {
        do {
            ...
            for (geom = gobject->geom; geom != (NuGeom*)0x0; geom = geom->next) {
            }
...

{
    ...
    if (nblends != 0) {
        __n = nblends << 2;
        blendgeom = (struct NuBlendShape*)NuMemAlloc(0x40);
